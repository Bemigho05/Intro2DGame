#include "Scene.h"
#include "../Engine/GameEngine.hpp"  // Now we have full access to GameEngine
...
void Scene::simulate(size_t frames) { /* Implementation */ }

void Scene::registerAction(int inputKey, const std::string& actionName) {
    m_actionMap[inputKey] = actionName;
}

float Scene::width() const { return static_cast<float>(m_game->window().getSize().x); }

float Scene::height() const { return static_cast<float>(m_game->window().getSize().y); }
